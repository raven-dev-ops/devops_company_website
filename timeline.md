...
- Introduce the Python FastAPI `chat-assistant-backend` on Cloud Run fronted by API Gateway.
- Align frontend wiring so the chatbot can talk to either backend via `VITE_CHAT_API_BASE`.
